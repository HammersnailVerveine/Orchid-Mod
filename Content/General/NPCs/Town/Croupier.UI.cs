...
namespace OrchidMod.Content.General.NPCs.Town
{
	...
	public class CroupierUI : ILoadable
	{
		...
		void ILoadable.Load(Mod mod)
		{
			...
			On_Player.SetTalkNPC += ModifySetTalkNPC;
			On_Main.GUIChatDrawInner += DrawOverVanillaNPCChat;
		}
		...
		private void DrawCardTooltips(SpriteBatch spriteBatch, Item hoverItem, int maxReq, bool canRemove)
		{
			...
			var tooltipsPosX = drawRect.X + 20 + cardSlotsTexture.Width();
			...
			spriteBatch.Begin(spriteBatchSnapshot with { RasterizerState = UISystem.OverflowHiddenRasterizerState });
		...
		private static void ModifySetTalkNPC(On_Player.orig_SetTalkNPC orig, Player player, int npcIndex, bool fromNet)
		{
		...
		private static void DrawOverVanillaNPCChat(On_Main.orig_GUIChatDrawInner orig, Main main)
		{
			var ui = Instance;
		...
		private static Rectangle GetClippingRectangle(Rectangle rect, SpriteBatch spriteBatch)
		{
			...
			rectangle.X = Utils.Clamp(rectangle.X, 0, width);
			rectangle.Y = Utils.Clamp(rectangle.Y, 0, height);
			rectangle.Width = Utils.Clamp(rectangle.Width, 0, width - rectangle.X);
			rectangle.Height = Utils.Clamp(rectangle.Height, 0, height - rectangle.Y);
