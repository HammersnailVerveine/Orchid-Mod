...
namespace OrchidMod.Content.Projectiles.Pets
{
			...
			if (Projectile.ai[0] <= 255)
			{
				if (Projectile.ai[0] >= 50)
				{
					if (Main.rand.Next(255 - (int)Projectile.ai[0]) <= 20)
					{
						...
						Main.dust[Dust.NewDust(pos, Projectile.width / 10, Projectile.height / 10, 269, 0f, 0f, 6, default, 0.8f)].velocity *= 0.1f;
					}
