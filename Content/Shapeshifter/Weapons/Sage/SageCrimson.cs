...
namespace OrchidMod.Content.Shapeshifter.Weapons.Sage
{
	public class SageCrimson : OrchidModShapeshifterShapeshift
	{
		...
		public override void ShapeshiftAnchorAI(Projectile projectile, ShapeshifterShapeshiftAnchor anchor, Player player, OrchidShapeshifter shapeshifter)
		{
			...
			{
				speedMult -= speedMult * 0.8f * (anchor.ai[0] / 300);
			}
			...
			if (anchor.IsLeftClick)
			{
				if (ShapeshiftCanLeftClick(projectile, anchor, player, shapeshifter))
				{
					...
					if (IsLocalPlayer(player))
					{
						...
						ShapeshifterNewProjectile(shapeshifter, projectile.Center + offSetSpawn, Vector2.Zero, projectileType, Item.damage * 0.3f, Item.crit, 0f, player.whoAmI, finalTargetLocation.X, finalTargetLocation.Y, ai2:1);
			...
			else if (anchor.ai[2] <= 0f)
			{
				...
				else if (IsGrounded(projectile, player, intendedDistance + 2.5f, anchor.IsInputDown, anchor.IsInputDown) && intendedVelocity.Y < 1f && shapeshifter.ShapeshifterMoveSpeedDecelerate != 0f)
