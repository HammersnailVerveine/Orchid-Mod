...
namespace OrchidMod.Content.Shaman.Projectiles.Equipment
{
	public class LavaDroplet : OrchidModShamanProjectile
	{
		...
		public override void OnKill(int timeLeft)
		{
			for (int i = 0; i < 2; i++)
			{
				int index2 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.Torch, Projectile.velocity.X, Projectile.velocity.Y, 100, new Color(), 1f);
