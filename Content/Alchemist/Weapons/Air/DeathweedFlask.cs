	...
	public class DeathweedFlask : OrchidModAlchemistItem
	{
		...
		public override void KillSecond(int timeLeft, Player player, OrchidAlchemist modPlayer, AlchemistProj alchProj, Projectile projectile, OrchidModGlobalItem globalItem)
		{
			...
			for (int i = 0; i < nb; i++)
			{
				...
				int spawnProj = ProjectileType<Content.Alchemist.Projectiles.Air.AirSporeProjAlt>();
			...
			for (int l = 0; l < Main.projectile.Length; l++)
			{
				...
				if (proj.active == true && proj.type == ProjectileType<Content.Alchemist.Projectiles.Air.AirSporeProj>() && proj.owner == projectile.owner && proj.localAI[1] != 1f)
				{
					proj.Kill();
				}
			}
			...
			for (int i = 0; i < nb; i++)
			{
				...
				SpawnProjectile(player.GetSource_Misc("Alchemist Attack"), projectile.Center, vel, ProjectileType<Content.Alchemist.Projectiles.Air.AirSporeProj>(), dmg, 0f, projectile.owner);
			}
		}
