	...
	public class RecipeSunflowerSeeds : AlchemistHiddenReactionRecipe
	{
		...
		public override void Reaction(Player player, OrchidAlchemist modPlayer)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				int spawnProj = ProjectileType<Content.Alchemist.Projectiles.Nature.SunflowerFlaskProj1>();
			...
			for (int i = 0; i < nb; i++)
			{
				...
				int spawnProj = ProjectileType<Content.Alchemist.Projectiles.Nature.SunflowerFlaskProj4>();
