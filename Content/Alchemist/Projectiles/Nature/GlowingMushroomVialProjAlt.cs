...
namespace OrchidMod.Content.Alchemist.Projectiles.Nature
{
	public class GlowingMushroomVialProjAlt : OrchidModAlchemistProjectile
	{
		...
		{
			Texture2D texture = ModContent.Request<Texture2D>("OrchidMod/Content/Alchemist/Projectiles/Nature/GlowingMushroomVialProjAlt_Glow").Value;
		...
		public override void AI()
		{
			...
			else if (Projectile.timeLeft % 60 == 0)
			{
				...
				if (sporeType == 6)
				{
					projType = ProjectileType<Content.Alchemist.Projectiles.Fire.FireSporeProj>();
				}

				if (sporeType == 59)
				{
					projType = ProjectileType<Content.Alchemist.Projectiles.Water.WaterSporeProj>();
				}

				if (sporeType == 61)
				{
					projType = ProjectileType<Content.Alchemist.Projectiles.Nature.NatureSporeProj>();
				}

				if (sporeType == 63)
				{
					projType = ProjectileType<Content.Alchemist.Projectiles.Air.AirSporeProj>();
				}
