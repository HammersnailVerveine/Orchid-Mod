	...
	public abstract class OrchidModAlchemistCatalyst : ModItem
	{
		...
		public virtual void GeneralCatalystInteractionEffect(Player player)
		{
			...
			if (modPlayer.alchemistFlowerSet)
			{
				...
				{
					Projectile.NewProjectile(null, player.Center.X, player.position.Y - 65, 0f, 0f, ProjectileType<Content.Alchemist.Projectiles.Reactive.BloomingReactive>(), 0, 0, player.whoAmI, 0f, 0f);
				}
				if (modPlayer.alchemistFlower >= 9)
				{
					modPlayer.alchemistFlower = 0;
					int dmg = (int)player.GetDamage<AlchemistDamageClass>().ApplyTo(25);
					Projectile.NewProjectile(null, player.Center.X, player.position.Y - 65, 0f, 0f, ProjectileType<Content.Alchemist.Projectiles.Reactive.BloomingReactiveAlt>(), dmg, 0, player.whoAmI, 0f, 0f);
				}
			}
		}
