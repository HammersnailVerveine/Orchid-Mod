...
{
	public class OceanCardBase : OrchidModGamblerProjectile
	{
		...
		{
			// DisplayName.SetDefault("Ocean Card Base");
		}
		...
		{
			Projectile.ai[0] = 10;
			bushTexture ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/OceanCardBase", AssetRequestMode.ImmediateLoad).Value;
			fruitTextureOutline ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/OceanCardProj_Outline", AssetRequestMode.ImmediateLoad).Value;
			fruitTexture ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/OceanCardProj", AssetRequestMode.ImmediateLoad).Value;
		...
		public override void SafeAI()
		{
			...
			if (Projectile.ai[0] <= 0) {
				Projectile.ai[0] = 90f;
				int projType = ProjectileType<Content.Gambler.Projectiles.OceanCardProj>();
			...
			if (Projectile.ai[1] == 0f)
			{
				if (Main.myPlayer == Projectile.owner)
				{
					...
					if (cardType != ItemType<Gambler.Weapons.Cards.OceanCard>() || modPlayer.gamblerShuffleCooldown <= 0 && !dummy || dummy && !modPlayer.GamblerDummyInHand)
					{
