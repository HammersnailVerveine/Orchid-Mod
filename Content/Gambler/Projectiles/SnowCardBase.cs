...
namespace OrchidMod.Content.Gambler.Projectiles
{
	public class SnowCardBase : OrchidModGamblerProjectile
	{
		...
		{
			Projectile.ai[0] = 10;
			bushTexture ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/SnowCardBase", AssetRequestMode.ImmediateLoad).Value;
			fruitTextureOutline ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/SnowCardProj_Outline", AssetRequestMode.ImmediateLoad).Value;
			fruitTexture ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/SnowCardProj", AssetRequestMode.ImmediateLoad).Value;
		...
		public override void SafeAI()
		{
			...
			if (Projectile.ai[0] <= 0) {
				Projectile.ai[0] = 90f;
				int projType = ProjectileType<Content.Gambler.Projectiles.SnowCardProj>();
			...
			if (Projectile.ai[1] == 0f)
			{
				if (Main.myPlayer == Projectile.owner)
				{
					...
					if (cardType != ItemType<Gambler.Weapons.Cards.SnowCard>() || modPlayer.gamblerShuffleCooldown <= 0 && !dummy || dummy && !modPlayer.GamblerDummyInHand)
					{
