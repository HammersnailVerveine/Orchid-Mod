...
namespace OrchidMod.Content.Gambler.Projectiles
{
	public class MushroomCardBase : OrchidModGamblerProjectile
	{
		...
		{
			Projectile.ai[0] = 10;
			bushTexture ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/MushroomCardBase", AssetRequestMode.ImmediateLoad).Value;
			fruitTextureOutline ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/MushroomCardProj_Outline", AssetRequestMode.ImmediateLoad).Value;
			fruitTexture ??= ModContent.Request<Texture2D>("OrchidMod/Content/Gambler/Projectiles/MushroomCardProj", AssetRequestMode.ImmediateLoad).Value;
		...
		public override void SafeAI()
		{
			...
			if (Projectile.ai[0] <= 0) {
				Projectile.ai[0] = 90f;
				int projType = ProjectileType<Content.Gambler.Projectiles.MushroomCardProj>();
			...
			if (Projectile.ai[1] == 0f)
			{
				if (Main.myPlayer == Projectile.owner)
				{
					...
					if (cardType != ItemType<Gambler.Weapons.Cards.MushroomCard>() || modPlayer.gamblerShuffleCooldown <= 0 && !dummy || dummy && !modPlayer.GamblerDummyInHand)
					{
