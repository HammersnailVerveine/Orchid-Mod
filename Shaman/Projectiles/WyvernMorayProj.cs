...
namespace OrchidMod.Shaman.Projectiles
{
	public class WyvernMorayProj : OrchidModShamanProjectile
	{
        ...
        {	
			...
			if (projectile.timeLeft % 5 == 0) spawnDustCircle(16, 10 + Main.rand.Next(10));
		}
		...
		public override void Kill(int timeLeft)
        {
			...
			spawnDustCircle(16, 40);
			Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 34);
			Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, 0f, 0f, mod.ProjectileType("WyvernMorayProjLingering"), (int)(projectile.damage * 0.8f), 0.0f, projectile.owner, 0.0f, 0.0f);
			
			for(int i=0; i<10; i++) {
				int dustType = Main.rand.Next(2) == 0 ? 15 : 16;
