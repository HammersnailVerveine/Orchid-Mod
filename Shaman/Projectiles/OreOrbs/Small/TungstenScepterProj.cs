...
namespace OrchidMod.Shaman.Projectiles.OreOrbs.Small
{
	public class TungstenScepterProj : OrchidModShamanProjectile
	{
		...
		public override void SafeOnHitNPC(NPC target, int damage, float knockback, bool crit, Player player, OrchidModPlayer modPlayer)
		{
			...
			{
				Projectile.NewProjectile(player.Center.X - 15, player.position.Y - 20, 0f, 0f, Mod.Find<ModProjectile>("EmeraldOrb").Type, 0, 0, Projectile.owner, 0f, 0f);

				if (player.FindBuffIndex(Mod.Find<ModBuff>("ShamanicBaubles").Type) > -1)
				{
					modPlayer.orbCountSmall++;
					Projectile.NewProjectile(player.Center.X, player.position.Y - 25, 0f, 0f, Mod.Find<ModProjectile>("EmeraldOrb").Type, 1, 0, Projectile.owner, 0f, 0f);
					player.ClearBuff(Mod.Find<ModBuff>("ShamanicBaubles").Type);
				}
			}
			if (modPlayer.orbCountSmall == 2)
				Projectile.NewProjectile(player.Center.X, player.position.Y - 25, 0f, 0f, Mod.Find<ModProjectile>("EmeraldOrb").Type, 0, 0, Projectile.owner, 0f, 0f);
