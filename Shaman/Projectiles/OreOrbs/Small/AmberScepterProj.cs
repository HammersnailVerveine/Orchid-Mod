...
namespace OrchidMod.Shaman.Projectiles.OreOrbs.Small
{
	public class AmberScepterProj : OrchidModShamanProjectile
	{
		...
		{
			OrchidModProjectile.spawnDustCircle(Projectile.Center, 64, 5, 8, true, 1.5f, 1f, 4f, true, true, false, 0, 0, true);
			OrchidModProjectile.spawnDustCircle(Projectile.Center, 64, 5, 8, true, 1.5f, 1f, 2.5f, true, true, false, 0, 0, true);
		...
		public override void SafeOnHitNPC(NPC target, int damage, float knockback, bool crit, Player player, OrchidShaman modPlayer)
		{
			...
			{
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X - 15, player.position.Y - 20, 0f, 0f, Mod.Find<ModProjectile>("AmberOrb").Type, 0, 0, Projectile.owner, 0f, 0f);

				if (player.FindBuffIndex(Mod.Find<ModBuff>("ShamanicBaubles").Type) > -1)
				{
					modPlayer.orbCountSmall++;
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X, player.position.Y - 25, 0f, 0f, Mod.Find<ModProjectile>("AmberOrb").Type, 1, 0, Projectile.owner, 0f, 0f);
					player.ClearBuff(Mod.Find<ModBuff>("ShamanicBaubles").Type);
				}
			}
			if (modPlayer.orbCountSmall == 2)
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X, player.position.Y - 25, 0f, 0f, Mod.Find<ModProjectile>("AmberOrb").Type, 0, 0, Projectile.owner, 0f, 0f);
			if (modPlayer.orbCountSmall == 3)
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X + 15, player.position.Y - 20, 0f, 0f, Mod.Find<ModProjectile>("AmberOrb").Type, 0, 0, Projectile.owner, 0f, 0f);

			if (modPlayer.orbCountSmall > 3)
			{
				player.AddBuff(Mod.Find<ModBuff>("AmberEmpowerment").Type, 60 * 30);
				modPlayer.orbCountSmall = 0;
			}
		}
