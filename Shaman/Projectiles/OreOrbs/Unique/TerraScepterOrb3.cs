...
{
	public class TerraScepterOrb3 : OrchidModShamanProjectile
	{
		float startX = 0;
		float startY = 0;
		int orbsNumber = 0;
		...
		public override void AI()
		{
			...
			if (player.GetModPlayer<OrchidModPlayer>().orbCountUnique == 0 || player.GetModPlayer<OrchidModPlayer>().orbCountUnique > 19 || player.GetModPlayer<OrchidModPlayer>().shamanOrbUnique != ShamanOrbUnique.TERRA)
				projectile.Kill();
		...
		public override void Kill(int timeLeft)
		{
			...
			if (player.GetModPlayer<OrchidModPlayer>().orbCountUnique < 20)
				Projectile.NewProjectile(projectile.position.X, projectile.position.Y, (Main.rand.Next(20) - 10) * 1f, -5f, mod.ProjectileType("TerraScepterOrbHoming3"), dmg, 0f, projectile.owner, 0f, 0f);
		}
	}
}
