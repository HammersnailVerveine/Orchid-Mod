...
{
	public class TerraScepterOrb1 : OrchidModShamanProjectile
	{
		float startX = 0;
		float startY = 0;
		int orbsNumber = 0;
		...
		public override void AI()
		{
			...
			if (player.GetModPlayer<OrchidModPlayer>().orbCountUnique == 0 || player.GetModPlayer<OrchidModPlayer>().orbCountUnique > 9 || player.GetModPlayer<OrchidModPlayer>().shamanOrbUnique != ShamanOrbUnique.TERRA)
				Projectile.Kill();
		...
		public override void Kill(int timeLeft)
		{
			...
			if (player.GetModPlayer<OrchidModPlayer>().orbCountUnique < 10)
				Projectile.NewProjectile(Projectile.position.X, Projectile.position.Y, (Main.rand.Next(20) - 10) * 1f, -5f, Mod.Find<ModProjectile>("TerraScepterOrbHoming1").Type, dmg, 0f, Projectile.owner, 0f, 0f);
		}
	}
}
